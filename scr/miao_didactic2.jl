...
# constraint (7) : original from Miao2009
#@constraint(mod, cst7_[i=1:n, j=1:n, k=1:m, l=1:m], f[i,j] * z[i,j,k,l] * (d[j] - a[i] -  t[k,l]) >= 0)

# constraint (7') : suggested by XG
@constraint(mod, cst7p_[i=1:n, j=1:n, k=1:m, l=1:m], z[i,j,k,l] * (d[j] - a[i] -  f[i,j] * t[k,l]) >= 0)
...
if termination_status(mod) == OPTIMAL
    ...
    println("Feasible transferts of pallet(s):")
    for i=1:n, j=1:n, k=1:m, l=1:m
