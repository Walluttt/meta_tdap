using JuMP, GLPK, Printf, TexTables, Gurobi
...
function model2015(filename)  
    
    println("\nModel_2015_with_Julia : \n")
    ...
    for w = 1:v
        ...
        @constraint(model, logicZ1[i in 1:n,j in 1:n, k in 1:m, l in 1:m], z[i,j,k,l] <= y[i,k])
        @constraint(model, logicZ2[i in 1:n,j in 1:n, k in 1:m, l in 1:m], z[i,j,k,l] <= y[j,l])
        @constraint(model, penalty[i in 1:n, j in 1:n,k in 1:m, l in 1:m; i!=j], y[i,k] + y[j,k] <= 1 + instance.x[i,j] + instance.x[j,i])
        @constraint(model, dock[i in 1:n, j in 1:n, k in 1:m, l in 1:m; i!=j], z[i,j,k,l] <= instance.x[i,j])
        ...
        @constraint(model, time_window[i in 1:n, j in 1:n,k in 1:m, l in 1:m; (instance.d[j]-instance.a[i]-instance.t[k,l]<=0) && i!=j], z[i,j,k,l] == 0)
        ...
        push!(results, (obj,trunc(t_TDAP, digits=3),number_of_delivery,delivery_done))
...
if (all)
    ...
    if (table)
        ...
        savefig(graph_mono(info,"model_mono_2015"),"fig/model_mono_2015.png")


        #latex = to_table(data_comp, t_value, t_time, t_cpt)
        #latex |> print
        #to_tex(latex) |> print
    end
    return
