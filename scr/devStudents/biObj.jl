...
function biObj(instancefile)
    ...
    while test
        ...
        #delete(mod, cstF1)
        #unregister(mod, cstF1)
        @constraint(mod, sum(1000 * t[k,l] * z[i,j,k,l] for i=1:n, j=1:n, k=1:m, l=1:m) <= f1-1)
        optimize!(mod)
